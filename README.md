...
The action does not install the Swift toolchain. We recommend using [`swift-install-action`](https://github.com/rarestype/swift-install-action) for this, but you can use any Swift setup action as long as you know where it installs the toolchain.
...
In this example, we are using [`swift-install-action`](https://github.com/rarestype/swift-install-action), which provides an environment variable `SWIFT_INSTALLATION` that we can pass to `--swift-toolchain`.
...
If you want to use a different Swift toolchain than the pre-installed one, you could use [`swift-install-action`](https://github.com/rarestype/swift-install-action) in combination with `--swift-toolchain $SWIFT_INSTALLATION`.
